...
export const RELAY_API = process.env.RELAY_API ?? 'http://127.0.0.1:8080';
export const WORKER_NAME = flag('--name') ?? process.env.RELAY_NAME ?? `worker-${process.pid}`;
export const WORKER_CAPACITY = num(flag('--capacity') ?? process.env.RELAY_CAPACITY, 4);
